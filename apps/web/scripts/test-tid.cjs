...
const {TID_ALPHABET, localAnchor, orientation, FINGERS, samplePose} = require('../src/components/tid/poses.ts');
...
const centreX = (hand, side) => {
  const q = orientation(hand.rotation);
  // Compare the hand bodies. In E the three extended fingers cross to the
  // left of the other hand; their tips must not reverse the reported hand order.
  const p = localAnchor(hand.shape, 'palm');
  if (side === 'left') p.x *= -1;
  return p.applyQuaternion(q).x + hand.position[0];
};
for (const letter of TID_ALPHABET.filter(l => l.pose.right.visible && l.pose.left.visible)) {
  // Y's requested 180° turns put the left palm to the right of its upright
  // index, crossing the hand bodies while the fingertip stays at the V roots.
  const order = letter.ch === 'Y' ? -1 : 1;
  assert(order * (centreX(letter.pose.right, 'right') - centreX(letter.pose.left, 'left')) > 0,
    `${letter.ch}: hand bodies must retain the requested screen order`);
}
