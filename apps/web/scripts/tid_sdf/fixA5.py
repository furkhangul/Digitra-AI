...
RROT=[0,0,156]; LROT=[0,0,90]        # mirror of the solved pose: legs=right, bar=left
...
def measure(spread,gap):
    fk,R,L,BAR,A1,A2=setup(spread,gap)
    bardir=BAR[1]-BAR[0]
    ...
    return dict(gN=dN-(RB+RI), gF=dF-(RB+RM), tN=tN, tF=tF, sF=sF,
                ...
                hands=(fk,R,L))
def cost(x):
    spread=x[0]; m=measure(spread,x[1:4])
...
for s in range(14):
    ...
    x0=np.concatenate([[rng.uniform(5,12)],[0.327,-0.033,-0.239]+rng.uniform(-.25,.25,3)])
...
sh.save('sheet/A-mirrored.png'); print('rendered')
